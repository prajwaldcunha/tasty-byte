...
<?php	else:
		$city = cleanInput($_GET['city']);
		$sql  = "SELECT products.id as pid,name,imageurl,details,quantity,price,manufacturedate,pickup_address,products.city,fname,lname,email,phoneno FROM products, users WHERE products.uid=users.id AND products.city='".$city . "' AND soldout = 0";
		$result = $conn->query($sql);
		?>
